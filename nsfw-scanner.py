...
def parse_args() -> tuple[str, bool, Optional[str]]:
    ...
    parser.add_argument('-t', '--target', type=str, default=None, help='Target directory where NSFW files will be moved')
    ...
    for file in files:
        ...
        if file.lower().endswith(('.jpg', '.jpeg', '.png', '.gif')):
            ...
            if nsfw > 0.05 and nsfw < 0.15:
                print(image_path + ' MAY have NSFW content with a mixed score of nsfw: ' + str(nsfw) + ' and normal: ' + str(normal) + '. A manual review is suggested.')
                ...
                if target:
                    target_path = os.path.join(target, file)
                    os.rename(image_path, target_path)
                    print (f"{image_path} has NSFW content. Moving to {target_path}.")
                else:
                    print (f"{image_path} has NSFW content.")
            elif normal > 0.9:
                print(image_path + ' is ok.')
            else:
                print(image_path +" is ambiguous " + str(nsfw) + " vs " + str(normal) + ". A manual review is suggested.")
